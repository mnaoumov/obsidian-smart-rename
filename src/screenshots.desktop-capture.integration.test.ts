...
async function submitRenamePrompt(newTitle: string): Promise<void> {
  await evalInObsidian({
    async callback({ app, lib: { waitUntil }, newTitle: title, referenceNotePath: notePath }) {
      /*
       * The two waits and both sleeps share ONE eval, capped at ~30s: 2s + 10s + 10s + 1.2s. The rename and
       * the rewrite each land in well under a second, so halving the old single 20s ceiling costs nothing.
       */
