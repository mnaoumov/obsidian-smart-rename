...
- On mobile, a prompt's controls are not where they end up when the prompt first appears. Obsidian opens `.modal` shifted down by `translateY(341px)` and holds it there for a variable 300-600 ms before sliding it up. The harness's mobile `clickElement` aims at the rect as it reads it, and the tap can take over a second to arrive. A tap aimed during the hold lands on `.modal-bg` and closes the prompt, so the checkbox "never unticks" however long you wait. Wait until `.modal`'s **computed** `transform` is `none` and the target's rect has held still, as `rename-checkboxes` does. The inline style is not a reliable signal: the shift has been seen with no inline style at all.
