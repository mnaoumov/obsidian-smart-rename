...
- On mobile, a prompt's controls are not where they end up when the prompt first appears. Obsidian opens `.modal` shifted down by `translateY(341px)` and holds it there for a variable 300-600 ms before sliding it up. The harness's mobile `clickElement` aims at the rect as it reads it, and the tap can take over a second to arrive. A tap aimed during the hold lands on `.modal-bg` and closes the prompt, so the checkbox "never unticks" however long you wait. obsidian-integration-testing 17's `clickElement` now waits for this itself - until nothing is animating the target or an ancestor and its box has held still - and refuses a covered centre with a named error, so `rename-checkboxes` waits only for the prompt to exist. If a tap still lands on `.modal-bg`, suspect the harness wait before re-adding one here; the inline style is not a reliable signal, since the shift has been seen with no inline style at all.
